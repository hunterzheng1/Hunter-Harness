...
export async function locateRecovery(
  ...
  const index = await readIndex(options.recoveryRoot);
  ...
  for (const entry of candidates) {
    ...
    try {
      ...
      if (!await containedRegularFileExists(options.recoveryRoot, journalPath) ||
          !await containedRegularFileExists(options.recoveryRoot, mirrorPath)) {
        continue;
      }
