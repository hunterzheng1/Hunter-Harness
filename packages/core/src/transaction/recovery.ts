...
function assertV3Identity(
  ...
  if (required.some((value) => value === null || value === undefined ||
    ...
    throw new RecoveryPreconditionError(
      ...
      "schema v3 recovery identity is incomplete"
    );
  }
  const comparisons: Array<[string | undefined, string | null | undefined]> = [
    [options.projectIdentity, journal.project_identity],
    [options.cliVersion, journal.cli_version],
    [options.targetBundleVersion, journal.target_bundle_version],
    [options.ownershipManifestHash, journal.ownership_manifest_hash],
    [options.expectedPlanHash, journal.plan_hash]
  ];
  if (comparisons.slice(0, 4).some(([expected]) =>
  ...
  if (comparisons.some(([expected, actual]) =>
    expected !== undefined && expected !== actual
  )) {
    throw new RecoveryPreconditionError(
      "RECOVERY_PRECONDITION_FAILED",
      "recovery identity or immutable plan does not match"
    );
  }
...
async function assertRecoveryPayloads(
  ...
  if (location.source === "project") {
    ...
    if (digest !== journal.snapshot_digest) {
      throw new RecoveryPreconditionError(
        "RECOVERY_PRECONDITION_FAILED",
        "recovery snapshot digest does not match"
      );
    }
    ...
    if (mirroredSnapshots.length !== mirroredPaths.size ||
      ...
      throw new RecoveryPreconditionError(
        "RECOVERY_PRECONDITION_FAILED",
        "durable recovery snapshot digest does not match"
      );
    }
  }
