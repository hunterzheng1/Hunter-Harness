...
async function assertRollbackPreconditions(
  ...
  try {
    if (location.source === "project") {
      if (journal.schema_version === 3) {
        if (typeof journal.snapshot_digest !== "string" ||
          ...
          throw recoveryPrecondition("recovery snapshot digest does not match");
        }
      }
      return;
    }
    ...
    if (mirroredSnapshots.length !== mirroredPathSet.size ||
      ...
      throw recoveryPrecondition("durable recovery snapshot digest does not match");
    }
  } catch (error) {
...
export async function runTransaction(
  ...
  try {
    ...
    try {
      ...
      await writeTransactionJournal(transactionRoot, journal);
      if (options.recoveryStore !== undefined) {
        await syncDurableRecovery(projectRoot, journal, options.recoveryStore);
      }
      for (let index = 0; index < operations.length; index += 1) {
        const operation = operations[index];
        if (operation === undefined) {
