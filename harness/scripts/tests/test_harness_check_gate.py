...
class CheckGateTests(unittest.TestCase):
    ...
    def _write_marker(
        self, ts: float | None = None, tree: str = "tree123", command: str = "npm run check"
    ) -> None:
        if ts is None:
            ts = time.time()
        self.marker.write_text(
            json.dumps({"ts": ts, "treeHash": tree, "command": command}) + "\n",
            encoding="utf-8",
        )

    def _run_gate(self, tree: str = "tree123") -> int:
        with mock.patch.object(gate, "MARKER", self.marker), mock.patch.object(
            gate, "_current_head_tree", return_value=tree
        ):
            return gate.main([])

    def test_skip_when_marker_matches_head(self) -> None:
        # fresh ts + matching HEAD + matching command → skip (exit 0)
        self._write_marker(tree="tree123")
        self.assertEqual(self._run_gate(tree="tree123"), 0)

    def test_run_when_no_marker(self) -> None:
        self.assertEqual(self._run_gate(tree="tree123"), 1)

    def test_run_when_marker_stale(self) -> None:
        # older than MAX_AGE_S → run (exit 1)
        self._write_marker(ts=time.time() - gate.MAX_AGE_S - 1, tree="tree123")
        self.assertEqual(self._run_gate(tree="tree123"), 1)

    def test_run_when_committed_tree_differs(self) -> None:
        self._write_marker(tree="tree123")
        self.assertEqual(self._run_gate(tree="tree999"), 1)

    def test_run_when_command_mismatch(self) -> None:
        self._write_marker(tree="tree123", command="npm test")
        self.assertEqual(self._run_gate(tree="tree123"), 1)
    ...
    def test_write_marker_writes_current_head_then_skips(self) -> None:
        ...
        with mock.patch.object(gate, "MARKER", self.marker), mock.patch.object(
            gate, "_current_index_tree", return_value="treefeed"
        ), mock.patch.object(gate, "_current_head_tree", return_value="treefeed"):
            self.assertEqual(gate.main(["--write"]), 0)
            data = json.loads(self.marker.read_text(encoding="utf-8-sig"))
            self.assertEqual(data["treeHash"], "treefeed")
