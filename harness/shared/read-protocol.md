...
6. **implementation-detail** — 自适应执行参考：v2 读 `plans/<change>-design.md` 的 `# Implementation Detail` 节；legacy 读独立 `plans/<change>-implementation-detail.md`，缺失 🟡WARN，不阻断
7. **test-scenarios** — 测试真相源：v2 读 `plans/<change>-plan.md` 的 `# Test Scenarios` 节；legacy 读独立 `plans/<change>-test-scenarios.md`
