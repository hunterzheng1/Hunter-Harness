...
| 选 v2 还是 legacy | **计划里存在 `requiredEvidenceKind=ledger` 的场景（P0/P1 通常都是）时必须走 legacy。** v2 派生的 `meta/scenario-manifest.json` 是 artifact 包装体，缺 `priority`/`requiredEvidenceKind`/`ownerPhase`/可执行测试三元组，run/test 门禁与 `harness_ledger.py record` 都会 fail-closed 报 `SCENARIO_MANIFEST_V2_UNSUPPORTED`——发布能成功，后面的证据闭环走不通。判据与字段对照 → `reference.md`「已知缺口」 |
