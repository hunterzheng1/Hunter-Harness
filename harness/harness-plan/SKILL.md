...
| 1 | **条件触发，不每次必查**：需求涉及历史取舍/设计原因、兼容或升级边界、疑似与既有变更重复，或用户要求延续既有方案时，直接执行一次 `npx hunter-harness knowledge query "<用户需求原文>" --limit 10 --json`；全新独立需求跳过本步直接进阶段 2（2026-09 审查实测：无条件查询的自然语言原文 8/8 零命中，强制执行只有成本没有收益）。这是唯一执行入口，不扫描技能目录、不查找其他脚本；失败记 `issue` 并继续，不建立本地索引或离线回退。**查询结果（含 0 条）必须落事件**：`harness_events.py append --phase plan --type command --command "knowledge query" --note "count=<n>" --json`（`command` 类型事件只接受 `command`/`exit_code`/`duration_ms`/`note` 字段，多给 `--name`/`--message` 会被 `EVENT_FIELD_NOT_ALLOWED` 拒绝；query 回执的 `receipt.index_generation` 反映索引代数；0 条时可先跑 `npx hunter-harness knowledge status --json`（0.4.11+）区分「job 未跑 / 失败 / 结果为空」）。**standard/full 档（09-M4 门禁，fail-closed）**：查询改为必查（用目标/领域词/模块线索构造结构化查询词，不整段原文直查），且完整 `--json` 输出必须落收据锚点——plan 关门逐条校验结构、项目身份绑定与预算，缺锚点、伪造或跨项目收据直接拒绝关门；0 命中同样满足（门禁查的是「执行并留证」不是「必须命中」）。留证：`python <skills-root>/scripts/harness_plan_finalize.py record-knowledge-receipt --change-dir ".harness/changes/<cn>" --payload-file <query.json> --json`（锚点 `meta/knowledge-query-receipt.json`，按 query_id 幂等 upsert，最多 2 条，第二条须为 `reason_code=directed_evidence_followup` 定向追查——当前 CLI 每次查询均为 initial_intent，追查入口开放前每个 change 只记一条；fast 档豁免并在关门 payload 记录跳过原因；降级开关 `knowledgeGateMode=warn`——项目 `.harness/config/gate-policy.json`、change `meta/gate-policy.json` 或 env `HUNTER_HARNESS_KNOWLEDGE_GATE_MODE`） |
| 2 | 歧义优先检查 + 复杂度分级；先确认会改变实现方向的语义歧义。**需求澄清（10-M3，standard/full 档 fail-closed，fast 豁免）**：任务/场景写进 `meta/plan-evidence-input.json` 后先跑确定性静态检查（零 LLM）：`python <skills-root>/scripts/harness_clarify.py check --change-dir ".harness/changes/<cn>" --json`——查目标/任务目标存在、task_refs/requirement_refs 闭合、acceptance 与 evidence 非空，有缺陷先修输入再重跑。静态全绿后由你做**一次** LLM 歧义扫描（脚本不调 LLM，只留证），把确认清单落证：`... record-scan --change-dir ".harness/changes/<cn>" --payload-file <scan.json>`（载荷 `{"schema_version":1,"questions":["…"]}`，≤5 条；零疑问给空列表；同问题集幂等重放返回 `CLARIFY_SCAN_REPLAYED`）。逐条向用户确认后闭包：`... confirm --change-dir ".harness/changes/<cn>" --payload-file <answers.json>`（载荷 `{"schema_version":1,"answers":[{"confirmation_id":"clarify_confirm:<64hex>","answer":"…"}]}`，id 取自 record-scan 回执）。收据为 `meta/clarify-report.json`（v1，哈希绑定当时 PEI）；之后改过 PEI 必须重跑 `check` rebase，否则关门报 `CLARIFY_GATE_REPORT_STALE` |
...
| 设计真相源 | **v2** = `plans/<cn>-design.md`（finalize 派生、哈希绑定、六 target 之一）；**legacy** = `spec/<cn>-design.md`。同一 change 只有一份设计权威，禁止两处并存导致漂移；下游读取顺序见 `shared/read-protocol.md` |
...
| 知识查询 | 阶段 1 失败不得假装已读历史，也不得改用本地索引或其他执行入口；standard/full 档 plan 关门以 `meta/knowledge-query-receipt.json` 锚点为证（09-M4，fast 豁免），锚点只由 `harness_plan_finalize.py record-knowledge-receipt` 写入，不得手拼 |
| 需求澄清门禁 | standard/full 档 plan 关门以 `meta/clarify-report.json`（v1）为证（10-M3，fast 豁免）：静态检查全绿 + 扫描已留证 + 确认清单全部 answered 才放行；门禁每次全量重跑静态检查，报告哈希不绑定当前 PEI 即 `CLARIFY_GATE_REPORT_STALE`（重跑 `harness_clarify.py check`），有待答确认即 `CLARIFY_GATE_CONFIRMATIONS_OPEN`。报告只由 `harness_clarify.py check/record-scan/confirm` 写入，不得手拼。回滚开关 `clarifyGateMode=off`（env `HUNTER_HARNESS_CLARIFY_GATE_MODE` > change `meta/gate-policy.json` > 项目 `.harness/config/gate-policy.json`）；存量无报告且静态干净的 change 按 `not_required` 兼容放行 |
