...
| 发布后改产物 | 改 `meta/plan-evidence-input.json` 后重跑 `plan publish`——`expected_baseline`（上次 manifest 哈希/generation）与 `context.attempt` 递增都由命令从 journal/plan-events 自动记账，不再手填；收据过期加 `--renew-review`。分解链下重跑 `plan evidence-pack` 时仍需手工递增 attempt、置 present baseline。**绝不手改 `meta/scenario-manifest.json`**（派生物，手改必致 `ARTIFACT_HASH_DRIFT`）→ `reference.md`「发布后修订计划」 |
