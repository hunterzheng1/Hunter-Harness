...
0. 加载上下文：**推荐路径（批次 2 WI-2）**——一条命令 `harness_context.py bootstrap-execute --project . --change <id> --executor <tool> --json`（内部完成 prepare → context begin 交接校验 → gate begin 含测试基线 guard；幂等：重跑复用同一 runId；foundation-gate pending 时需带 `--task <n>`）。返回 `EXECUTE_BOOTSTRAPPED` 即进入步骤 0.5；返回 `BOOTSTRAP_EXECUTE_*_FAILED` 时按信封内 `recoveryAction` 排障（原三连命令路径保留为出口）：`harness_context.py prepare --project . --change <id> --phase execute --executor <tool> --json` → `harness_context.py begin --project . --change <id> --phase execute --executor <tool> --json` → `harness_gate.py begin --phase execute --change <id>`。旧名 `--phase run` / `--phase test` 仍被接受并归一为 execute。<br>Git Bash 注意：`--note` 等字符串参数**不要以 `/` 开头**（MSYS 会把它当路径改写成 `C:/Program Files/Git/...`）；以 `/` 开头时先 `MSYS_NO_PATHCONV=1`。<br>**交接凭证不必手工补**：v2 计划的 `plan finalize` 不写 context 事务，`prepare` 会在检测到 committed 的 `meta/publication-journals/*.json` 时自动补录 `plan → execute` 凭证（凭证带 `bootstrapSource=plan_publication_journal` 留痕）。仍报 `HANDOFF_REQUIRED`/`LEGACY_BOOTSTRAP_REQUIRED` 说明**没有**这份发布证据——回到 plan 阶段确认发布是否真的完成，**不得**自己拼 `classify + configure-plan + close` 造凭证。<br>**Fixback** 不得拼装底层步骤，必须只调用一次 `harness_fixback.py launch-review --project . --change <id> --change-dir <change-dir> --executor <tool> --skills-root <skills-root> --product-identity <当前产品身份> --json`；返回 `FIXBACK_NOTHING_TO_APPLY` 时直接报告并停止。禁止手写 `events.ndjson` / `phase.end`。<br>**失败断路器**：bootstrap-execute 前按失败指纹检查最近连续 ≥2 次 FAIL，命中返回 `EXECUTE_CIRCUIT_OPEN` 阻断——先按信封 `recoveryAction` 排查根因；确属与本变更无关的预存失败才用 `--circuit-ack "<理由>"` 放行（信封 `circuitBreaker.overridden` 留痕）。
...
0.6. **波次派发（advisory，16-M1/16-M2）**：bootstrap 信封带 `scenarioWaves`（拓扑波次）与 `waveDispatch`（含完成集进度的派发计划）。`waveDispatch.available=true` 时按 `runnableNow` 波次升序派发：同一波内**不在同一 `conflictGroups` 组**的场景可用 Agent 工具并行分派实现（写面不重叠）；冲突组（共享同一 `testFile`）内串行或交同一执行者；`blocked` 场景待其 `pendingDeps` 完成后自动解锁；`deferred` 场景按计划留给后续阶段。每波完成并把验证写入 ledger 后，重跑 `harness_context.py execute-wave --project . --change <id> --json`（只读，无副作用）取下一波派发计划。`available=false`（无清单/畸形/依赖成环）或旧版本无此命令时退回既有顺序执行——波次是调度建议不是门禁，关门校验不受影响。并行只作用于实现/分析：**验证执行仍统一走 `harness_test_runner.py exec`**（项目级单实例锁），不得并行起构建
...
**中断恢复（批次 2 WI-3）**：会话中断后重进 execute，先跑一条只读命令看「现在在哪、下一步做什么」——`harness_change.py status --change <id> --json`（返回 `CHANGE_RECOVERY_VIEW`：当前阶段/runId/attempt、plannedPhases 进度、ledger 验证状态、租约、脏树、`nextAction`）。`handoffPending=true` 表示 phase.end 已写但交接未落盘——按 `nextAction` 用原 close 命令补 `--to-phase` 幂等续跑。**不要**法证式逐个读 gate-policy/events/ledger/state-snapshot 原文来拼状态；视图只读派生同一批权威文件，不产生新状态。
...
| **Gate/Guard** | execute 入口一条命令 `harness_context.py bootstrap-execute`（三连命令保留为排障出口）；跨 Agent/阶段先用 `harness_context.py prepare/begin`；阶段门禁统一用 `harness_gate.py begin/close`；gate 内部负责 guard begin/close（`harness_test_guard.py begin` / `harness_test_guard.py close` 仅由 gate 调用，不构成模型执行步骤），模型只在需要记录测试来源或修复时调用 guard 的 `record/stage/mark` |
