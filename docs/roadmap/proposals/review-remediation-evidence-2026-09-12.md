...
- 现状（2026-09-18 更新）：**已立项设计**，见 [design-o5-benefit-evidence-2026-09-18.md](./design-o5-benefit-evidence-2026-09-18.md)。
  - Part A（现有试点报告范围校正）：**已执行**——三份试点报告（`batch0/pilot-g3`、`batch2/pilot-close`、`batch3/collected/t5-full-tier`）头部加「证据范围校正」块，越界表述（「清零」「全部达成」「质量零回退」）限定为实际样本范围，负结果逐项保留（Harness 稳态慢于原生、质量维度无优势证据、P12 未达成、T5 未覆盖、apiTest DEGRADED）；两处下游设计文档加指针。
  - Part B（新对照实验）：**未执行**——协议已定稿（§3：3 组 × 6 类 × 3 重复、盲评分离、版本冻结、目标 -20%/-50% 锁定），待执行窗口（WI-O5.2~O5.5）。
- 状态：**部分**（校正完成、实验未执行；原因：实验需独立工作区/盲评人力/多轮窗口，属专项排期）。
...
- **未执行检查及原因**：① O5 新对照实验（协议已定稿，执行窗口/盲评人力未排期，见 O5 设计文档 WI-O5.2~O5.5）；② 真实 GitHub Actions 全量绿收据（本证据以本地聚焦套件 529 项全绿代替：task 81 + ledger 110 + review 68 + archive 155 + O3/O4/O6 批次 115）；③ 大规模增量评审漏审率对照（并入 O5 域）。

## 剩余阻塞

- O5（收益证据受控实验）——已立项设计（Part A 校正完成、Part B 协议定稿待执行，2026-09-18），不阻塞本任务书其余检查点；实验执行需独立排期。
