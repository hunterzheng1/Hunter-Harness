...
2026-09-12 审核后的完整实施入口见 [审核整改与产品闭环任务书](proposals/review-remediation-execution-2026-09-12.md)，包含五项缺陷、六项优化、实施提示词和独立复审要求；当前为待实施，不代表相关目标已完成。

产品方向讨论见 [高效交付与自动资产沉淀草案](proposals/product-optimization-proposal-2026-09.md)。该草案的三项产品决策已于 2026-09-07 敲定（个人与小团队优先；事实+经验自动、规则与可执行资产受控；新任务默认完整任务入口、保留显式阶段调用），批次 0 基线方法与任务集见 [批次 0：任务成本基线与对照方法](batches/batch0-baseline-and-method.md)。草案其余部分仍不替代下面的既有实施合同。
...
| 顺序 | 文档 | 主要交付物 | 状态 |
|---:|---|---|---|
| 01 | [内容边界与协议](./stages/01-content-boundaries-and-contracts.md) | 同步范围、内容类型、状态和 API 契约 | 已关闭（01-M1、01-M2 验收通过） |
| 02 | [远端同步核心](./stages/02-remote-sync-core-and-branch-snapshots.md) | Core 深模块、分支快照、版本与冲突算法 | 实施中（02-M1/M2、Platform 分支快照读模型 M3～M6 与 PostgreSQL Adapter M7 已关闭；Harness CLI 已有 actor-bound HTTP transport，Platform 生产装配、Pull 工作区与页面仍待接入） |
...
| 20 | [验收测试防篡改](./stages/20-acceptance-test-tamper-guard.md) | 修复轮次触碰 plan 声明验收测试文件须人工 ack（确定性阻断）、calibrate 不代为重录 | 已实施（20-M1，2026-09-18） |
...
- `docs/roadmap/**` 必须受 Git 跟踪，不能只存在于本地忽略目录。
