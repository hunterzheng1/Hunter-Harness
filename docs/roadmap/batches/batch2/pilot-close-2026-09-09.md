...
> **证据范围校正（2026-09-18，审核整改任务书 §11 / O5）**：本文结论仅限 2 次完整流程运行（T4'/T6'）、
> 单 agent 单会话、暖缓存、无独立工作区隔离。「四门槛全部达成」限定为该样本范围，不构成统计证据；
> **T5（full 档完整流程）未覆盖**（§5 自述）。保留的负结果：B2-5 tier/mode 双轨（standard 声明实际走
> assurance 四阶段）、B2-6 恢复路径唯一偏离（读 events 原文取 runId）、T6' 恢复链 4 次失败重试。
> 冻结版本：clone @ 41d2887；原始数据：本文件即采集记录。
