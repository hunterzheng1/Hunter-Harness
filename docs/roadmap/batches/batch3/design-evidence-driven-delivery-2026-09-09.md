...
> 日期：2026-09-09
>
> 状态：**已裁决（2026-09-09）**。§0 四项建议经用户确认全部采纳，
> 快车道（B2-1/2/3/4/6 + B2-5 止血告警 + apiTest 探针）启动实施；
> WI-1/2/3 按本文顺序推进。
>
...
**裁决记录**：2026-09-09 用户确认四项建议值（①B2-5 分两步：止血告警进快车道，对齐为 WI-1；②T5 在 WI-1 后补测，apiTest 探针前置到快车道；③批次 3 按提案原范围推进，B2 系列修复作为前置快车道；④B2-2 走 publish 内透传）。快车道自本记录起实施。
...
> 注（2026-09-18）：批次 2 试点结论（「四门槛达成」）的样本范围，见
> `batch2/pilot-close-2026-09-09.md` 头部「证据范围校正」块（审核整改任务书 §11 / O5）。
...
> 注（2026-09-18）：T5' 为 n=1 单运行试点，「五门槛全部达成」的样本范围见
> `collected/t5-full-tier-2026-09-10.md` 头部「证据范围校正」块（审核整改任务书 §11 / O5）。
