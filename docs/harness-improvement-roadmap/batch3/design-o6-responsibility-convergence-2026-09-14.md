...
> 状态：**F1 已实施（2026-09-14）**；§0 决策点已由用户裁决采纳建议值；
> F2～F6 待实施。
...
| 6 | `dependsOn` 三处同名异义 | A. gate DAG 依赖改名（task.json dependsOn 保留）　B. 不改 | **A**（task.json 的 dependsOn 是 WI-3.3 发布的用户面契约，不动；gate DAG 内部依赖与 ledger :124 字段改语义化命名，纯内部重构） |
...
- **判定**：✅ 单一权威。⚠️ 遗留：`dependsOn` 三处同名异义
  （task.json 并行依赖 / gate DAG 依赖 gate.py:1458 / ledger :124）→ 决策点 6。
...
- **写入方**：`harness_gate.py`（权威加载/分类/覆盖；写 gate-policy 于 :4174）+
  **`harness_task.py:2247` 跨模块调 `hg._write_json` 私有函数二次写 gate-policy**。
- **判定**：🔴 **gate-policy.json 双写入方 + tier 四处落盘**——O6 最直接的
  「同一事实多个权威写入方」实例 → F2。
...
| # | 候选 | 证据 | 处置 WI |
|---|------|------|---------|
| 1 | gate-policy.json 双写入方 | gate.py:4174 vs task.py:2247（私有函数跨用） | F2 |
...
| 9 | dependsOn 三处同名异义 | task.json / gate.py:1458 / ledger :124 | F2（决策点 6） |
| 10 | 收据五载体无登记 | §1.5 表 | F4（决策点 5） |
| 11 | outbox 门面寄居 assets | assets.py:190/:203 | F6（决策点 7） |
| 12 | tier 四处落盘 | risk-classification/gate-policy/task.tier/declaredTier | F2 |
...
| **F2 tier 单一权威** | gate-policy 单写入方；tier 四处落盘收敛为「一处权威 + 投影」；gate DAG 依赖改名消 dependsOn 歧义 | 内部重构 + 命名 | F1 |
